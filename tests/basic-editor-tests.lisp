...
(test first-scrolling
  ...
  (let ((experimental-window (main :testing T))
    ...
    (is (= 0 (first-line (view-port model))))
    (is (= 0 (first-column (view-port model))))
    ...
    (is (= 1 (first-line (view-port model))))
    (is (= 0 (first-column (view-port model))))
    (is (= 1 (be::row (nth 5 (char-kids model)))))

    (process-event experimental-window  :key-pressed '("n" "n" 57 (:ctrl)))
    (is (= 2 (first-line (view-port model))))
    (is (= 0 (first-column (view-port model))))
    ...
    (is (= 1 (first-line (view-port model))))
    (is (= 1 (first-column (view-port model))))
    (is (= 6 (be::col (nth 5 (char-kids model)))))
    (is (= 1 (be::row (nth 5 (char-kids model)))))

    (process-event experimental-window  :key-pressed '("f" "f" 41 (:ctrl)))
    (is (= 1 (first-line (view-port model))))
    (is (= 2 (first-column (view-port model))))
...
(test single-line-one-character-with-newline
  ...
  (with-fixture prepare-text-no-window ((format nil "A~%"))
    ...
    (is (eq t    (be::valid-cursor-position model 0 0)))
    (is (eq nil  (be::valid-cursor-position model -1 -1)))
    (is (eq nil  (be::valid-cursor-position model 0 -1)))
    (is (eq nil  (be::valid-cursor-position model -1 0)))
    (is (eq t    (be::valid-cursor-position model 0 1)))
    (is (eq nil  (be::valid-cursor-position model 1000 0)))
    (is (eq nil  (be::valid-cursor-position model 0 1000)))
    (is (eq nil  (be::valid-cursor-position model 0 2)))
    (is (eq nil  (be::valid-cursor-position model 1 0)))))
